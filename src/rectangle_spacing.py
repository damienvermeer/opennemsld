...
def space_rectangles(
    ...
    # No need to reinitialize shifts - they're already set correctly above
    ...
    while iteration < max_iterations:
        ...
        # Create shapely polygons for current positions
        polygons = []
        for x1, y1, x2, y2 in current_positions:
        ...
        for i in range(len(polygons)):
            for j in range(i + 1, len(polygons)):
                ...
                # Check for intersection
                    ...
                    _apply_direction(directions, i, angle_j_to_i)  # Move i away from j
                    _apply_direction(directions, j, angle_i_to_j)  # Move j away from i
        ...
        if debug_images:
            _generate_debug_image(current_positions, iteration)
...
def _generate_debug_image(
    rectangles: List[Tuple[float, float, float, float]], iteration: int
) -> None:
